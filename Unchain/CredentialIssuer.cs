...
/// requested by a Unchain client. This means this component abstracts receives
...
/// proving to the Unchain client that the credentials were issued with the right
/// key).
///
...
public class CredentialIssuer
{
	...
	/// This parameter is called `k` in the original paper.
	/// </summary>
	...
	/// <exception cref="UnchainCryptoException">Error code: <see cref="UnchainCryptoErrorCode">UnchainErrorCode</see></exception>
	...
	public CredentialsResponse HandleRequest(ICredentialsRequest registrationRequest)
	{
		...
		{
			throw new UnchainCryptoException(
				UnchainCryptoErrorCode.InvalidNumberOfRequestedCredentials,
		...
		{
			throw new UnchainCryptoException(
				UnchainCryptoErrorCode.InvalidNumberOfPresentedCredentials,
		...
		{
			throw new UnchainCryptoException(UnchainCryptoErrorCode.InvalidBitCommitment);
		}
		...
		{
			throw new UnchainCryptoException(UnchainCryptoErrorCode.SerialNumberDuplicated);
		}
		...
		lock (SerialNumbersLock)
		{
			...
			{
				throw new UnchainCryptoException(UnchainCryptoErrorCode.SerialNumberAlreadyUsed, $"Serial number reused");
			}
		...
		try
		{
			...
			if (!areProofsValid)
			{
				throw new UnchainCryptoException(UnchainCryptoErrorCode.CoordinatorReceivedInvalidProofs);
			}
		}
		finally
		{
