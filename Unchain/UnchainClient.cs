...
/// <summary>
/// Provides the methods for creating <see cref="ICredentialsRequest">unified Unchain credential registration request messages</see>
...
public class UnchainClient
{
		...
		CancellationToken cancellationToken)
	{
		...
		{
			throw new UnchainCryptoException(UnchainCryptoErrorCode.CredentialToPresentDuplicated);
		}
		...
		CredentialsResponseValidation registrationValidationData)
	{
		...
		if (issuedCredentialCount != NumberOfCredentials)
		{
			throw new UnchainCryptoException(
				UnchainCryptoErrorCode.IssuedCredentialNumberMismatch,
		...
		{
			throw new UnchainCryptoException(UnchainCryptoErrorCode.ClientReceivedInvalidProofs);
		}
